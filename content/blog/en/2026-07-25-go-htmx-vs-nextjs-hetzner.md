...
The site you're reading right now is built with Next.js and hosted on Vercel. But if you flip the letter in the bottom-right corner to the other view, you can take a look at the second implementation. It's made with the Go+HTMX stack and hosted on my own Hetzner cloud server. The database is SQLite. Both are the same site — same landing page, same AI bouncer, same dashboard — but two completely different technology choices in one package. There are small differences in the site's design in some implementations, but nothing significant or affecting the site's performance.
